...
async function prepareNodeModules(entrypointPath, namespaceDir) {
  const modulesPath = path__default["default"].join(entrypointPath, "node_modules");
  try {
    const namespacedPath = path__default["default"].join(entrypointPath, namespaceDir);
    try {
    ...
    if (fs__default["default"].existsSync(namespacedPath)) {
      consola__default["default"].log(`Using cached ${namespaceDir}`);
      fs__default["default"].moveSync(namespaceDir, modulesPath);
    }
  } catch (e) {
    consola__default["default"].log(`Error creating ${namespaceDir}.`, e);
  }
}
async function backupNodeModules(entrypointPath, namespaceDir) {
  const modulesPath = path__default["default"].join(entrypointPath, "node_modules");
  try {
    const namespacedPath = path__default["default"].join(entrypointPath, namespaceDir);
    const stats = await fs__default["default"].stat(modulesPath);
    if (stats.isDirectory()) {
      await fs__default["default"].rm(namespacedPath, { force: true, recursive: true });
      await fs__default["default"].move(modulesPath, namespacedPath);
    }
  } catch (e) {
    consola__default["default"].log(`Error backing up node_modules to ${namespaceDir}.`, e);
  }
}
...
async function build(opts) {
  ...
  let publicPath = nuxtConfigFile.build && nuxtConfigFile.build.publicPath ? ufo.withTrailingSlash(ufo.withoutLeadingSlash(nuxtConfigFile.build.publicPath)) : nuxtConfigFile.router && nuxtConfigFile.router.base ? `${ufo.withTrailingSlash(ufo.withoutLeadingSlash(nuxtConfigFile.router.base))}_nuxt/` : "_nuxt/";
